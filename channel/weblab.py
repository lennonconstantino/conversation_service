...
class Weblab(Channel):
    ...
    def save_request(self, owner: MessageOwner, channel: str, client_hub: str, message: str, message_type: str, meta: Dict[str, Any] = None):
        # Usar consistentemente o método que retorna UUID
        ...
        # conversation_uuid já é string, passar diretamente
    ...
    def save_response(self, owner: MessageOwner, channel: str, client_hub: str, response: str, message_type: str, agent_type: str, meta: Dict[str, Any] = None):
        ...
        # Usar método consistente que retorna UUID
        conversation_uuid, is_new = self.service.get_or_create_conversation_uuid(client_hub=client_hub, channel=channel)
        if is_new:
            print("New conversation created for response")
